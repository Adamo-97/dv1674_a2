...
def find_latest_bench_folder():
    roots = [
        Path.cwd(),
        (Path(__file__).resolve().parent / ".." / "pearson").resolve()
    ]
    cands = []
    for r in roots:
...
def load_shell_agg(folder: Path):
    ...
    if seq_p.exists():
        ...
        seq["which"] = "seq"
        seq["app_label"] = "sequential (pearson)"
        seq["t_effective"] = 1.0
    if par_p.exists():
        par = pd.read_csv(par_p)
        par["which"] = "par"
        par["app_label"] = "parallel (pearson_par)"
    return seq, par
...
def plot_seq_dashboard(seq_df: pd.DataFrame, bench: Path, out_png: Path):
    ...
    # harmonize column naming to 'size'
    if "size" not in df.columns and "image" in df.columns:
        df["size"] = df["image"]
    ...
    def plot_row(ax, col, ylabel):
        ...
        for sz in sizes:
            ...
            y = sub[col].values
            dot = ax.plot(x, y, marker="o", linestyle="None", label=str(sz))[0]
            color = dot.get_color()
            # full-width horizontal line at the dot's y-value
...
def plot_par_dashboard(par_df: pd.DataFrame, bench: Path, out_png: Path):
    """
    Parallel: 3 metrics (Elapsed, RSS, CPU util) vs threads (log2), one line per size.
    ...
    def plot_metric(ax, col, ylabel):
        for sz in sizes:
            ...
            if sub.empty or col not in sub.columns:
                continue
            ax.plot(sub["threads"].astype(int), sub[col], marker="o", linestyle="-", label=str(sz))
        ax.set_xscale("log", base=2)
